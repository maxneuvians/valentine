defmodule ValentineWeb.WorkspaceLive.ImportComponentTest do
  ...
  alias ValentineWeb.WorkspaceLive.ImportComponent
  ...
  describe "render/1" do
    test "renders the form with an Import Workspace title", %{assigns: assigns} do
      assigns =
        %{
          assigns
  ...
  describe "handle_event/3" do
    ...
    test "save returns a flash info if the file is uploaded", %{socket: socket} do
      with_mock Phoenix.LiveView, [:passthrough],
        ...
        assert updated_socket.assigns.flash["info"] == "Workspace imported successfully"
        assert updated_socket.assigns.patch == "/workspaces"
      end
    end
